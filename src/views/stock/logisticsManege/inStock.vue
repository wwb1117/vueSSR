<template>
	<div>
		...
		<div class="model_content">
			<div class="model_content_inner" :style="{height: $store.state.home.modelContentHeight - 20 + 'px', overflow: 'auto'}">
				<div class="header_seach_wrap" style="padding: 15px">
					...
					<el-select filterable @change="supplierSelectChange" value-key="id" v-model="selectSupplierData" size="small" placeholder="请选择供应商">
						<el-option v-for="item in suppllerList" :key="item.id" :label="item.supplierName" :value="item"></el-option>
					</el-select>
				</div>
				<el-table
                    :data="tableData"
					...
					<el-table-column
                        ...
                        >
						<template slot-scope="scope">
							<el-input  @change.native="giftNumberKeyUp(scope.row)" @keyup.native="giftNumberKeyUp(scope.row)"  style="width: 100px" size="small" v-model="scope.row.giftNumber" placeholder="赠送量"></el-input>
						</template>
                    </el-table-column>
					<el-table-column
                        prop="unitName"
					...
					<el-table-column
                        ...
                        >
						<template slot-scope="scope">
							<el-input maxlength="10" @change.native="unitPriceKeyUp(scope.row)" @keyup.native="unitPriceKeyUp(scope.row)"  style="width: 100px" size="small" v-model="scope.row.unitPrice" placeholder="进货价"></el-input>
						</template>
                    </el-table-column>
					<el-table-column
                        prop="myTotal"
		...
		<div class="model_footer">
			...
			<el-button @click="sureInstockBtn" type="primary" size="small" style="width: 90px; margin-left: 25px;">确认进货</el-button>
		</div>
		...
		<el-dialog
			...
			<el-form :inline="true" class="myForm" label-position="right" size="small" label-width="90px">
				<div class="dialog_item" v-for="(item, index) in this.editTableData.cargoFlowNumList" :key="index">
					...
					<el-form-item prop="massage" label="对应进货量">
						<el-input @change.native="cargoFlowNumberItemKeyUp(item)" @keyup.native="cargoFlowNumberItemKeyUp(item)" v-model="item.cargoFlowNumber" placeholder="请输入对应进货量"></el-input>
					</el-form-item>
				</div>
		...
		<el-dialog
			...
			<el-form v-model="postData" label-position="right" size="small" label-width="90px">
				<div class="dialog_item">
					<el-form-item prop="prepayMoney" label="预付款">
						<el-input @change.native="prepayMoneyKeyUp(postData)" @keyup.native="prepayMoneyKeyUp(postData)" v-model="postData.prepayMoney" placeholder="请输入输入预付款">
	...
	export default {
		data() {
			return {
				...
				postData: {
					...
					supplierId:'',
					supplierName: '',
					inShopId: this.$store.state.home.currentShop.id,
					inShopName: this.$store.state.home.currentShop.shopName,
		...
		methods: {
			supplierSelectChange(){
				this.postData.supplierId = this.selectSupplierData.id
				this.postData.supplierName = this.selectSupplierData.supplierName
			},
			cateSelectChange(){
			...
			prepayMoneyKeyUp(data){
				data.prepayMoney = this.myBase.formatFloatTwo2(data.prepayMoney)
			},
			unitPriceKeyUp(data){
			...
			giftNumberKeyUp(data){
				data.giftNumber = this.myBase.formatIntNum(data.giftNumber)
			},
			goBackEvent(){
				this.$router.push({
			...
			chooseGoodBtn(){
				...
				this.getGoodList()
				this.$nextTick(() => {
					this.$refs.instockGoodTable.clearSelection()
				})
			},
			getGoodList(){
			...
			resinStockSureEvent(status){
				...
				api.addInstockOrder(this.postData).then((response) => {
					this.$message({
						type: 'success',
						duration: 1500,
						showClose: true,
